...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/14 16:18:32 by jguigli           #+#    #+#             */
/*   Updated: 2022/03/14 16:18:34 by jguigli          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_collectible(t_data *game)
{
	...
	if (game->collectible_count != game->check_map.collectible)
		ft_printf("Collectible collected, remains %d\n",
			game->check_map.collectible - game->collectible_count);
	else
		ft_printf("You have collected all the collectibles, you can now exit\n");
}
...
void	move_player(t_data *game, char pos, int dir)
{
	...
	{
		mlx_put_image_to_window(game->mlx_ptr, game->win_ptr,
			game->character_back.mlx_img, (game->pos_x * 32),
			(game->pos_y * 32));
	}
	if (pos == 'y' && dir == DOWN)
	{
		mlx_put_image_to_window(game->mlx_ptr, game->win_ptr,
			game->character_front.mlx_img, (game->pos_x * 32),
			(game->pos_y * 32));
	}
	if (pos == 'x' && dir == LEFT)
	{
		mlx_put_image_to_window(game->mlx_ptr, game->win_ptr,
			game->character_left.mlx_img, (game->pos_x * 32),
			(game->pos_y * 32));
	}
	if (pos == 'x' && dir == RIGHT)
	{
		mlx_put_image_to_window(game->mlx_ptr, game->win_ptr,
			game->character_right.mlx_img, (game->pos_x * 32),
			(game->pos_y * 32));
	}
}
